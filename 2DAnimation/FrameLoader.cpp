...
void FrameLoader::setPath(char* path)
{
	this->path = path;
}

bool FrameLoader::loadContainer(IFrameContainer & container)
{
	...
	if (res)
	{
		...
		container.clear();
		readFrame(container.addEmpty());
		if (container.getCurrent() == nullptr)
		{
			container.clear();
			fm.closeInputFile();
			res = false;
...
std::shared_ptr<Frame> FrameLoader::readFrame(std::shared_ptr<Frame> frame)
{
	unsigned int id;
...
FrameLoader::dPoint FrameLoader::readPoint()
{
	bool success;
	long resID = -1;
...
bool FrameLoader::loadPoints(std::shared_ptr<Frame> frame)
{
	int size;
...
bool FrameLoader::loadPoint(std::shared_ptr<Frame> frame, FrameLoader::dPoint point)
{
...
FrameLoader::dEdge FrameLoader::readEdge()
{
	bool success;
...
bool FrameLoader::loadEdges(std::shared_ptr<Frame> frame)
{
	int size;
	if (!fm.readFromFile(size))
		frame = nullptr;
...
bool FrameLoader::loadEdge(std::shared_ptr<Frame> frame, FrameLoader::dEdge edge)
{
