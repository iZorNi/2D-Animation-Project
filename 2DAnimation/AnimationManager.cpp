...
{
	return saver->saveAnimation(path, std::weak_ptr<IFrameController>(container));
}
bool AnimationManager::load(std::string path)
{
	return loader->loadAnimation(path, std::weak_ptr<IFrameController>(container));
}
...
{
	container->getCurrentFrame().lock()->removeEdgeByCoord(x, y);
}
...
{
	container->getCurrentFrame().lock()->pickPoint(x,y);
}
...
void AnimationManager::freePickedPoint()
{
	container->getCurrentFrame().lock()->freePickedPoint();
}